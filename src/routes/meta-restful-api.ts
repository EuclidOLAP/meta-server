...
router.get("/dashboards", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch dashboards", error });
  }
});
...
router.get("/adhoc-queries", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching AdhocQueries", error });
  }
});
...
router.get("/dimension/:gid/hierarchies", async (req, res) => {
  ...
  try {
    ...
    // 构建返回的层级列表
    const hierarchyList = (hierarchies as any[]).map((hierarchy: any) => ({
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching hierarchies", error });
  }
});
...
router.get("/hierarchy/:gid/members", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching members", error });
  }
});
...
router.get("/cube/:gid/capacity", async (req, res) => {
  ...
  try {
    ...
    dimensionRoles = dimensionRoles.filter((dr: any) => {
      dr = dr.dataValues;
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching cube capacity", error });
  }
});
...
router.post("/cube/:gid/generate-measures", async (req, res) => {
  ...
  class OlapVectorGenerator {
    ...
    constructor(expectedMeasureRecords: any) {
  ...
  const do_generate_measures = async (
    ...
    const matchingFiles = files.filter((file: any) =>
      file.startsWith(cubeGid.toString())
    );
    if (matchingFiles.length > 0) {
      throw new Error(
        `文件已存在，请手动删除以 ${cubeGid} 为前缀的文件后再试。`
      );
    }
  ...
  try {
    await do_generate_measures(cubeGid, expectedMeasureRecords);
...
const createChildMember = async (
  ...
  if (parent.level === 0) {
    ...
    childFullPathBuffer.writeBigUInt64LE(
      BigInt(child.gid),
      parentFullPath.length
    ); // 将 child 的 gid 写入 buffer 的后 8 字节

    // 检查是否超出最大字节数
    if (childFullPathBuffer.length > 460) {
      throw new Error(
        "Full path exceeds the maximum allowed size of 460 bytes."
      );
    }
...
const createMembersTree = async (
  parent: any,
  children_fragments: any,
  transaction: any
) => {
  // createChildMember
  for (const child_fragment of children_fragments) {
    let new_member = await createChildMember(
      parent,
...
const do_create_dimension = async ({
  name,
  ...
  try {
    ...
    await dimension.update(
      { defaultHierarchyGid: default_hierarchy.gid },
      { transaction }
    );
...
router.post("/cube", async (req, res) => {
  ...
  try {
    ...
    const { dimension, default_hierarchy, root_level, root_member }: any =
      await do_create_dimension({
        name: "Measures",
...
router.post("/dimension", async (req, res) => {
  ...
  const result = await do_create_dimension({
    name,
    defaultHierarchyName,
    levels,
    membersTree,
  ...
  if (result) {
    ...
    res
      .status(500)
      .json({ error: "Failed to create dimension with related data" });
  }
});
...
router.post("/child-member", async (req, res) => {
  ...
  const transaction = await sequelize_conn.transaction();
  try {
    const newChildMember = await createChildMember(
      parentMember,
      newChildMemberName,
      transaction
    );
...
router.get("/calculatedMetrics/cube/:cube_gid", async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      metrics: calculatedMetrics.map((metric: any) => {
        // ({ olapEntityType: 'CalculatedMetric', ...metric.dataValues })
        const metricData = metric.dataValues;
        if (!metricData.display) metricData.display = metricData.name;
...
router.get(
  "/dimension/:gid/default-hierarchy-root-member",
  async (req, res) => {
    const dimensionGid = req.params.gid;
    ...
    try {
      ...
      if (!dimension) {
        return res
          .status(404)
          .json({ success: false, message: "Dimension not found" });
      }
      ...
      if (!defaultHierarchy) {
        return res
          .status(404)
          .json({ success: false, message: "Default Hierarchy not found" });
      }
      ...
      if (!rootLevel) {
        return res
          .status(404)
          .json({ success: false, message: "Root Level not found" });
      }
      ...
      if (!rootMember) {
        return res
          .status(404)
          .json({ success: false, message: "Root Member not found" });
      }
      ...
      res
        .status(500)
        .json({ success: false, message: "Error fetching root member", error });
    }
  }
);
...
router.post("/adhoc-query", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error creating AdhocQuery", error });
  }
});
